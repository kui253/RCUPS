...
class DecoderPrefixAtten(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            if prefix_value is not None:
                # 这里还需要进行映射
                key_prefix, value_prefix = prefix_value.split(1)
                key_states = torch.cat([key_prefix.squeeze(0), key_states], dim=2)
                value_states = torch.cat([value_prefix.squeeze(0), value_states], dim=2)
...
class DualAttenModelOutput(ModelOutput):
    ...
    high_encoder_last_hidden_state: torch.FloatTensor = None
    high_encoder_attention_mask: torch.LongTensor = None
    project_context: Tuple[torch.FloatTensor] = None
...
class BartDecoderWithDualCrossAttention(BartDecoder):
    ...
    def forward(
        ...
        **kwargs,
        ...
        projected_context = kwargs["project_context"]
        output_attentions = (
            output_attentions
            if output_attentions is not None
            else self.config.output_attentions
        ...
        if (
            ...
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            # 添加上prefix的维度
            low_encoder_attention_mask = torch.concat(
                [
                    torch.ones(input_shape[0], self.n_slots).to(
                        low_encoder_attention_mask.device
                    ),
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    past_key_value=past_key_value,
                    output_attentions=output_attentions,
                    prefix_value=(projected_context[idx]),
                    use_cache=use_cache,
                    **kwargs,
                )
            hidden_states = layer_outputs[0]
...
class BartDecoderLayerWithDualCrossAttention(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.encoder_attn = DecoderPrefixAtten(
            self.embed_dim,
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not hasattr(self, "second_crossattn"):
            self.second_crossattn = copy.deepcopy(self.encoder_attn)
        low_encoder_last_hidden_state = kwargs["low_encoder_last_hidden_state"]
        low_encoder_attention_mask = kwargs["low_encoder_attention_mask"]
        ...
        if low_encoder_last_hidden_state is not None:
            ...
            (
                hidden_states,
                cross_attn_weights,
                cross_attn_present_key_value,  # bs head seqlen dim/heads
            ) = self.encoder_attn(
                hidden_states=hidden_states,
            ...
            # present.shape bs heads tgtlen dim/heads
        # second_crossattn
...
class DualAttenEncoder(BartEncoder):
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.context_attn_layer_norm = nn.LayerNorm(config.d_model)
        ...
        # self.layers_importance = copy.deepcopy(self.layers[: self.split_encoder_layers])
        self.layers = nn.ModuleList(
            [BartEncoderLayer(config) for _ in range(config.encoder_layers)]
        )  # 12 层
    ...
    def get_prompt(self, hidden_states):
        ...
        prompted = nn.functional.dropout(
            prompted, p=self.dropout, training=self.training
        )
        prompted = prompted.permute([2, 0, 3, 1, 4]).split(
            2
        )  # 沿着第一个维度切割成大小为2的tensor，然后有12个，也就是decoderlayer个
        return prompted
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        if not hasattr(self, "fusion_attn"):
            self.fusion_attn = copy.deepcopy(self.layers[4].self_attn)

        sorted_input_ids = kwargs.get("sorted_input_ids", None)
        sorted_attention_mask = kwargs.get("sorted_attention_mask", None)

        sent_input_ids = kwargs.get("sent_input_ids", None)
        sent_attention_mask = kwargs.get("sent_attention_mask", None)
        importance_label = kwargs.get("importance_label", None)
        len_ls = kwargs.get("len_ls", None)

        max_utt_num_in_a_batch = max(len_ls)
        bs = input_ids.shape[0]
        device = (
            sent_input_ids[0].device
            if isinstance(sent_input_ids, list)
            else sent_input_ids.device
        )
        ...
        # 搜sent_input_ids 的部分
        sent_hidden_state = self.get_low_encoder_output(
            sent_input_ids, sent_attention_mask, with_grad=True
        ).last_hidden_state
        d_model = sent_hidden_state.shape[-1]
        low_encoder_output_batched = self.extract_from_utt(
            sent_hidden_state, gt_attention_mask=sent_attention_mask
        )
        index_ls = []  # 这个应该是为了batch做准备，复制batch个
        for num_utt in len_ls:
        ...
        high_encoder_output_batched = (
            low_encoder_output_batched * importance_label[:, :, None]
        )
        high_encoder_attention_mask_batched = torch.tensor(
            [[1] * x + [0] * (max_utt_num_in_a_batch - x) for x in len_ls]
        ).to(device)
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (
                ...
                if idx < self.split_encoder_layers:
                    ...
                    if idx == self.split_encoder_layers:  # 只在相等的时候进行fusion
                        residual = normal_hidden_state
                        fusion_output = self.fusion_attn(
                            hidden_states=imptance_hidden_state,
                            key_value_states=normal_hidden_state,
                            attention_mask=normal_attention_mask,
                        )
                        fusion_state = fusion_output[0]
                        fusion_hidden_states = 0.7 * residual + 0.3 * fusion_state
                        hidden_states = torch.concat(
                            [fusion_hidden_states, imptance_hidden_state], dim=1
                        )
                    layer_outputs = encoder_layer(
                        hidden_states,
                        # normal_attention_mask,
        ...
        projected_context = self.context_attention(
            hidden_states=self.slot_embed.unsqueeze(0).expand(
                bs, self.n_slots, d_model
            ),
            key_value_states=imptance_hidden_state,
            attention_mask=_expand_mask(
                sorted_attention_mask,
                imptance_hidden_state.dtype,
        ...
        prompted = self.get_prompt(projected_context)
        return DualAttenModelOutput(
            low_encoder_last_hidden_state=hidden_states,
            low_encoder_attention_mask=concat_attention_mask,
            high_encoder_last_hidden_state=high_encoder_output_batched,
            high_encoder_attention_mask=high_encoder_attention_mask_batched,
            project_context=prompted,
        )
    ...
    def _expand_inputs_for_generation(  # generate step 2 在初始化beam_scorer之后
        ...
        **model_kwargs,
        ...
        if is_encoder_decoder:
            ...
            if "importance_hidden_state" in encoder_outputs.keys():
                encoder_outputs["importance_hidden_state"] = encoder_outputs[
                    "importance_hidden_state"
                ].index_select(0, expanded_return_idx.to(device))
            if "importance_attention_mask" in encoder_outputs.keys():
                encoder_outputs["importance_attention_mask"] = encoder_outputs[
                    "importance_attention_mask"
                ].index_select(0, expanded_return_idx.to(device))
            if "project_context" in encoder_outputs.keys():
                encoder_outputs["project_context"] = tuple(
                    [
                        prefixitem.index_select(1, expanded_return_idx.to(device))
                        for prefixitem in encoder_outputs["project_context"]
                    ]
                )
